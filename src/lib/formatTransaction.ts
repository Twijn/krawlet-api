...
export const parseTransactionData = (transaction: Transaction): TransactionData => {
    ...
    const errorEntry = meta.entries
        ?.find(x => x.name.toLowerCase() === "error");
    const messageEntry = meta.entries
        ?.find(x => x.name.toLowerCase() === "message");
    const blankEntry = meta.entries
        ?.find(x => !x.value);
...
export default (transaction: Transaction, data?: TransactionData): string => {
    ...
    return `<gray>${transaction.id.toLocaleString()}.</gray> ${data.from} <gray>-></gray> ` +
        `${data.to} <gray>|</gray> ` +
