...
export function validateShopSyncData(data: any): { isValid: boolean; errors: string[] } {
  ...
  if (
    data.info.multiShop !== undefined &&
    data.info.multiShop !== null &&
    (typeof data.info.multiShop !== 'number' || !Number.isInteger(data.info.multiShop))
  ) {
  ...
  if (data.info.software !== undefined) {
    if (typeof data.info.software !== 'object') {
      ...
      if (
        data.info.software.name !== undefined &&
        data.info.software.name !== null &&
        typeof data.info.software.name !== 'string'
      ) {
        errors.push("Field 'info.software.name' must be a string when provided");
      }
      if (
        data.info.software.version !== undefined &&
        data.info.software.version !== null &&
        typeof data.info.software.version !== 'string'
      ) {
  ...
  if (data.info.otherLocations !== undefined) {
    if (!Array.isArray(data.info.otherLocations)) {
      ...
      if (
        typeof data.info.otherLocations === 'object' &&
        Object.keys(data.info.otherLocations).length === 0
      ) {
...
function validateLocation(location: any, fieldPath: string): string[] {
  ...
  if (
    ...
    !['overworld', 'nether', 'end'].includes(location.dimension)
  ) {
    errors.push(
      `Field '${fieldPath}.dimension' must be "overworld", "nether", or "end" when provided`,
    );
  }
...
function validateItem(item: any, fieldPath: string): string[] {
  ...
  if (!item.item || typeof item.item !== 'object') {
    ...
    if (
      item.item.nbt !== undefined &&
      item.item.nbt !== null &&
      typeof item.item.nbt !== 'string'
    ) {
...
function validatePrice(price: any, fieldPath: string): string[] {
  ...
  if (
    price.requiredMeta !== undefined &&
    price.requiredMeta !== null &&
    typeof price.requiredMeta !== 'string'
  ) {
