...
export class KnownAddress
  extends Model<InferAttributes<KnownAddress>, InferCreationAttributes<KnownAddress>>
  implements RawKnownAddress
{
...
export const updateShopAddress = async (
  ...
  if (address.length === 0 || address.length !== 10 || address.includes(' ')) {
    ...
    throw new Error(
      'Description can only contain letters, numbers, spaces and basic special characters!',
    );
  }
