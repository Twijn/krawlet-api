...
export const extractMetaname = (transaction: TransactionWithMeta, name: string): string|null => {
    return transaction.meta?.entries?.find(entry => entry.name.toLowerCase() === name.toLowerCase())?.value ?? null;
}
...
const listener: Listener = async (transaction): Promise<ListenerResult> => {
    ...
    if (shopName || shopDescription) {
        if (!shopName) {
            ...
            return {success: false, message: "You can't assign a shop to your ReconnectedCC address! (Make a new wallet!)"};
        }
        try {
