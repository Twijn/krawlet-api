...
const command: Command = {
  ...
  execute: async (cmd: ChatboxCommand) => {
    ...
    if (showAll) {
      ...
      if (!player) {
        rcc
          .tell(cmd.user, "<red>We couldn't retrieve your player (REPORT THIS!)!</red>")
          .catch(console.error);
        return;
      }
    ...
    rcc
      .tell(
        cmd.user,
        `[View on Kromer.club](https://www.kromer.club/${player ? `addresses/${player.kromerAddress}/transactions` : 'transactions'})`,
        undefined,
        'markdown',
      )
      .catch(console.error);
